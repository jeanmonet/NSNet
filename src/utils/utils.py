...
def write_dimacs_to(n_vars: int, clauses: list[ClauseT], out_path):
...
def parse_cnf_file(file_path):
    ...
    for line in lines[i+1:]:
        ...
        if len(tokens) < 2 or tokens[0] == 'c' or tokens[0] == 'p':
            # undefined symbol, comment, duplicate header
            continue
...
def remove_dumplicate_literals(clauses: list[ClauseT]):
    return [list(set(clause)) for clause in clauses]


def unit_propagation(clauses: list[ClauseT]) -> tuple[list[ClauseT], set[int]]:
    fixed_vars = set()
    while True:
        need_propagate = set()  # type: set[int]
        assign = {}
        for clause in clauses:
            if len(clause) == 1:
...
def remove_fixed_vars(
        n_vars: int,
        clauses: list[ClauseT],
        fixed_vars: set[int]
) -> tuple[int, list[ClauseT]]:
    shifts = []
    shift = 0
...
def add_unused_variables(n_vars, clauses: list[ClauseT]) -> list[ClauseT]:
    used_vars = set()
    new_clauses = []
    for clause in clauses:
...
def preprocess(file_path) -> tuple[int, list[ClauseT]]:
...
def VIG(n_vars, clauses: list[ClauseT]) -> nx.Graph:
    G = nx.Graph()
